...
var imageTool = (function(mui, com) {
	...
	var itool = {
		options: {
			uploadUrl: 'http://192.168.0.117:8082/re/onload', //图片上传地址
	...
	function imgUploadCom(path, cb) {
		...
		var task = plus.uploader.createUpload(uploadUrl, {
			...
			//console.log("result:"+services.Decrypt(t.responseText));
			
		  if (status == 200) {
			
			var result=JSON.parse(services.Decrypt(t.responseText));
		...
		task.setRequestHeader("Content-type","Application/JSON; charset=utf-8");
		task.setRequestHeader("uuid", "phone");
		task.start();
	}
	/**
	 * @description 缩放图片
